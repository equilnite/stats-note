...
7. We now have all the components and the final task is to sum up the values.
    - Two options:
        ...
        - `2nd > stat (list) > MATH > sum(` and than just input your desired list to suum (which in the example would be `sum(L4)`).

Here's a [video tutorial](https://youtu.be/ZZtsomC2-Fg) using the example from Page 3 in your packets
