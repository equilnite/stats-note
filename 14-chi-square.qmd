...
**Hypotheses**: $H_0$: There is no association between the group and the variable, or the distribution of the variable is the same across all groups/treatments.
...
**Expected Counts**: The expected count for each cell ($E_{i,j}$) is the total count for the corresponding row ($\sum_{j = 1}^c x_{i,j}$) times the total count for the corresponding column ($\sum_{i = 1}^r x_{i,j}$) divided by the total count ($\sum_{j = 1}^c \sum_{i = 1}^r x_{i,j}$).
...
**Degrees of freedom**: We calculate the degrees of freedom by taking the \# of rows - 1 times the \# of columns - 1. $$df=(r - 1) \cdot (c-1)$$
