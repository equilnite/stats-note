...
By z-scores are also called standardized scores because they force a transformation of a data set to have a mean of 0 and standard deviation of 1. This allows values from different data sets to be comparable. This is similar what we need to do when comparing currencies between time periods. For example, thinking about whether \$1 in 2001 was more valuable than \$1 in 1992. In order to make that comparison, we would have to standardize the values so that they are comparable.

If we take two different values from different data sets or different contexts, **z-scores allow us to compare those two values because they "standardize" the values for easy comparison**.

::: {.callout-warning}
z-scores are first and foremost **standardized scores**. When you see a z-score, do not automatically assume that you have to use the normal distribution. This applies to every situation in this class. Without information on how the data is distributed, you *cannot* use a normal distribution. In order to use a method, you have to be able to imply or explicitly read that a specific method can be applied/used.
:::
