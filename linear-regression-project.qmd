...
3. The attached Google Doc that has some things for you to answer: 
    ...
    >
    > 1. *Take a moment to open up your one-hot encoded data file.* Explain what one-hot encoding does (hint: your original column(s) has been split up into multiple columns now–compare the original to the new columns). Provide an example of the original data and what the one hot encoded values for that original data now looks like.
    >
    ...
    >    b. Interpret the first “slope,” 1. It’s now slightly different from what we have before (it’s no longer, “for every 1…”).
